...
save_file_to = '/data/Adoption_data_new/result/active.json'
...
def get_all_developer(all_commit_history, projects_name, time):
...
for project in tqdm(project_list):
    ...
    for adoption in adoptions[project]:
        ...
        for time_stamp in interval:
            ...
            all_members = get_all_developer(author_commit_histroy[project], project, time_stamp)

            number_of_active_members = len(active_members)

            number_of_all_members = len(all_members)

            # print(active_members)

            if number_of_active_members == 0:
                ...
                '''
                if number_of_all_members == 0:

                    print([number_of_active_members, 1])

                else:

                    print([number_of_active_members, number_of_all_members])
            ...
            exposure_dict[project][tool].append(number_of_active_members/number_of_all_members)
